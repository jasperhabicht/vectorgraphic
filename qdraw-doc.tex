...
Similar to Ti\emph{k}Z, the \macro{qdraw} package makes use of key-value pairs to set options for commands. Some options are available for almost all commands, other have very specific uses. In the descriptions of the options, this manual states which data type the relevant option assumes. The following data types exist:
...
\begin{longtable}{ @{} p{2.5cm} p{\dimexpr\textwidth-\tabcolsep-2.5cm} @{} }
  ...
  \macro{color} & This means a color definition as specified by the \macro{l3color} module. Note that colors defined by the \macro{xcolor} package have no effect when using the \macro{qdraw} package. \\
...
foo
\begin{qdraw}[overlay, baseline={-0.5cm}]
...
\end{|scope|}
\end{macrodef}
The \macro{scope} environment is only available inside the \macro{qdraw} environment. It can be used to create scopes for the localization of path states, style settings and clipping regions. 
...
|(|<name>|)|
\end{macrodef}
Points on the path can finally be referenced using names. Points on the path can be named by either using the \macro{point} or the \macro{node} operation with the given name set via the \macro{nname} option (see \hyperlink{operation:point}{below}).
...
\end{macrodef}
Using the \macro{frame} command, the frame surrounding the node text can be selected. Currently, a rectangular and an elliptical frame are available selectable via the values \macro{rectangle} and \macro{ellipse}. The rectangular frame is set per default. All options that are available for paths are also available for node frames.
...
\end{macrodef}
The command \macro{\qdrawsetcolor} is used to define colors using the \macro{l3color} module. It is recommended to define custom colors in the preamble of the document. If the model is set to \macro{named}, the command expects as value a color expression.
...
\end{macrodef}
Arrow tips can be styled using the options \macro{arrow start style} and \macro{arrow end style}. The option \macro{arrow style} sets the same style for start and end arrow tips. All options that are available for paths are also available for arrow tips.

\begin{codeexamplecolumns}
\begin{qdraw}
  \stroke[
    arrow end, 
    arrow style={fill, fill color=blue}
  ]
    (0cm,0cm) -- (2cm,1cm) ;
\end{qdraw}
\end{codeexamplecolumns}

The following arrow tips are available, of these the last three, \macro{simple}, \macro{straight} and \macro{bar}, should be set together with the option \macro{arrow style={stroke}}

\NewDocumentCommand{\arrowtip}{ O{fill} m }{%
  \begin{qdraw}
    \stroke[arrow end={#2}, arrow style={#1}]
      (0cm,0cm) -- (1cm,0cm) ;
  \end{qdraw}%
}
